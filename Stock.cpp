...
public:
    Stock(const std::string &name, const float &value, bool rising, unsigned int uncertainty, unsigned int percentageChange)
        : name_(name), value_(value), rising_(rising), uncertainty_(uncertainty), percentageChange_(percentageChange){};

    Stock() : name_(""), value_(0), rising_(false), uncertainty_(0), percentageChange_(0){};
