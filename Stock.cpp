...
public:
  ...
  const bool &getRising() const { return rising_; }

  void setRising(bool isRising) { rising_ = isRising; }

  const unsigned int &getUncertainty() const { return uncertainty_; }

  const unsigned int &getPercentageChange() const { return percentageChange_; }
